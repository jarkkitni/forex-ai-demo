...
# (/demo/chat/rangkafae) เท่านั้น ไม่แตะ webhook Messenger จริงของร้านไหนเลย ----
...
# ---------- เสียง (STT+TTS) — เฉพาะ demo รังกาแฟ (23 ก.ค. 2026) ----------

def transcribe_audio(audio_bytes: bytes, mime_type: str) -> str:
    """ถอดเสียงลูกค้าเป็นข้อความผ่าน Gemini (native audio input — ไม่ต้องแยก STT provider ต่างหาก)
    ลองไล่ทีละโมเดลใน GEMINI_MODELS แบบเดียวกับ _call_gemini() แต่ตัดชั้น ListModels-discovery
    ออก (ฟีเจอร์ demo เดียว ไม่ต้องหนาเท่าทางตอบแชทหลัก) — ไม่ fallback ไป Groq/Claude เพราะไม่มี
    เจ้าไหนรับ audio input ราคาถูกเท่า Gemini ถ้า Gemini ตายให้ raise ตรงๆ ให้ผู้เรียกจัดการ (โชว์
    error ที่หน้า demo ได้เลย ไม่ต้องมี fallback ซับซ้อนสำหรับฟีเจอร์ทดลอง)"""
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY ยังไม่ได้ตั้ง")
    b64_audio = base64.b64encode(audio_bytes).decode("ascii")
    prompt = "ถอดเสียงภาษาไทยนี้เป็นข้อความล้วนๆ ห้ามอธิบายเพิ่ม ห้ามใส่เครื่องหมายคำพูด ตอบแค่ข้อความที่ถอดได้เท่านั้น"
    last_exc: Exception | None = None
    for model in GEMINI_MODELS:
        try:
            r = requests.post(
            ...
            print(f"[ai_guard] transcribe_audio model '{model}' ใช้ไม่ได้ ({e}) — ลองตัวถัดไป", flush=True)
            last_exc = e
